...
class _RestoreCutPipePageState extends State<RestoreCutPipePage> {
  ...
  @override
  void initState() {
    BlocProvider.of<RestoreCutPipeBloc>(context).add(
        RestoreCutePipePageLoadEvent(context: context));
    super.initState();
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: TextWidget("Restore Cut Pipe", color: AppColor.white,
  ...
  Widget _listBuilder({required FetchRestoreCutPipeDataState dataState}) {
               ...
               children: [
                 Expanded(
                     child: Column(
                       ...
                       children: [
                     TextWidget("Pipe Number : ${dataState.pipeList[index].pipeNumber}", fontWeight: FontWeight.w700,),
                     TextWidget("Pipe Length : ${dataState.pipeList[index].pipeLength}"),
                 ],)),
                 SizedBox(
                   ...
                   ButtonWidget(
                       ...
                       onPressed: () {
                         BlocProvider.of<RestoreCutPipeBloc>(context).add(RestoreCutePipeSubmitEvent(context: context, index: index));
    ...
    ): const Center(child: TextWidget("No Cute Pipe Data Found"),);
  }
