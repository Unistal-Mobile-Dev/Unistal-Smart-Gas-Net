...
import 'package:flutter_unistal_smart_gas_net/feature/steelStructure/addStreelStructure/helper/add_steel_structure_helper.dart';
import 'package:flutter_unistal_smart_gas_net/utils/commonClass/user_info.dart';
import 'package:intl/intl.dart';

part 'add_steel_structure_event.dart';
part 'add_steel_structure_state.dart';

class AddSteelStructureBloc extends Bloc<AddSteelStructureEvent, AddSteelStructureState> {
  ...
  AddSteelStructureBloc() : super(AddSteelStructureInitial()) {
    on<AddSteelStructurePageLoadEvent>(_pageLoadEvent);
    on<AddSteelStructureSelectAlignmentEvent>(_selectAlignment);
    on<SelectWeatherEvent>(_selectWeather);
    on<AddSteelStructureSelectDateEvent>(_selectDate);
    on<AddSteelStructureAddImageEvent>(_selectFile);
    on<AddSteelStructureSubmitDataEvent>(_submitData);
  }

  _pageLoadEvent(AddSteelStructurePageLoadEvent event, emit) async {
    emit(AddSteelStructurePageLoadState());
    dateController.text = "";
  ...
  _selectAlignment(AddSteelStructureSelectAlignmentEvent event, emit) {
    _alignmentData = event.alignmentData;
    _eventComplete(emit);
  }

  _selectDate(AddSteelStructureSelectDateEvent event, emit) async {
  ...
  _selectFile(AddSteelStructureAddImageEvent event, emit) async {
  ...
  _submitData(AddSteelStructureSubmitDataEvent event, emit) async {
    _isLoader =  true;
    _eventComplete(emit);
    var res =  await AddSteelStructureHelper.submitData(
  ...
  _eventComplete(Emitter<AddSteelStructureState>emit) {
    emit(FetchAddSteelStructureDataState(isLoader: isLoader,
