...
class AddTieinBloc extends Bloc<AddTieinEvent, AddTieinState> {
  ...
  _selectWPS(AddTieinSelectWPSEvent event, emit) async {
    ...
    _eventComplete(emit);
    var resWelder =  await AddWeldingHelper.fetchWelderData(context: event.context, userData: userData, wpsData: wpsData);
    if(resWelder != null){
  ...
  _submit(AddTieinSubmitDataEvent event, emit) async {
    ...
    var res = await AddTieinHelper.submitData(
      ...
      electrodeEiaE7010p1Batch: electrodeEiaE8010p1BatchController.text.toString(),
  ...
  _eventComplete(Emitter<AddTieinState>emit) {
    emit(FetchAddTieinDataState(isLoader: isLoader,
